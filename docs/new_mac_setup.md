...
高性能模式需由账号持有人核实微信公众平台后台状态；当前状态未确认。代码已保留两个 iOS 高性能开关，但代码配置不能证明后台能力已经开通。iOS 真机调试的 USB 要求与普通扫码预览是两条流程，详见[导出排错说明](wechat_export.md)。
...
- **尚未解决的真机结论：iPhone 16 Pro 通过加载页后画面空白，最新诊断包仍待用户反馈。** 不要把加载器修复或模拟器正常表述为 iPhone 已修复。
- 待核实：后台高性能模式；手机音效、前后台暂停、安全区、性能和不同平台的完整验收。
...
7. iPhone 空白尚未验收通过。需要继续排查时，使用 --diagnostics 生成新二维码并收集实际手机弹窗，不能以模拟器正常宣布 iPhone 已修复。
