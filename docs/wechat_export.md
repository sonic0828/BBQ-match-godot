...
截至 2026-09-25：Android 曾由用户反馈能进入并操作；iPhone 16 Pro 扫码预览仍空白，日志包含 ObjectDB、无效 Callable 和 UID 异常。用户已开通后台高性能模式，重新生成的诊断包待真机反馈。以下历史验证记录不能作为 iPhone 已修复的结论。
...
- 诊断现在保留最早 12 行错误、最多 64 种错误的次数与首次时间；诊断包合并重复输出，报告结束时重新输出完整 JSON。弹窗新增「复制诊断」，可直接粘贴反馈，无需连接 USB 或再次导出 vConsole。
